...
class PatchEmbed(nn.Module):
    def __init__(self, dim=24, embed_dim=96, patch_size=2):
        super().__init__()
        self.proj = nn.Conv2d(in_channels=dim, out_channels=embed_dim, kernel_size=patch_size, stride=patch_size)

    def forward(self, x):
        x = self.proj(x)#BCHW->B C H/p W/p
        x = x.flatten(2).transpose(1, 2) 
        return x

class PatchUnEmbed(nn.Module):
	def __init__(self, embed_dim=96, dim=24, patch_size=2):
		super().__init__()
		self.proj = nn.Sequential(
			nn.Conv2d(in_channels=embed_dim, out_channels=dim*patch_size**2, kernel_size=1, stride=1, padding=0),
...
def recover_from_diagonal_scan(results, B, C, H, W):
    L = H * W  # 总元素数
...
class Vim(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.patch_embed = PatchEmbed(
            patch_size=patch_size, in_chans=dim, embed_dim=dim)
        self.patch_unembed = PatchUnEmbed(
            patch_size=patch_size, embed_dim=dim, out_chans=dim)
        # Dropout
    ...
    def forward(self, x: Tensor, scan_mode: str):
        # Patch embedding
        B,C,H,W = x.shape
        # print(f"input: {x.shape}")
        x = self.patch_embed(x)
        if scan_mode == 'H':
            x = x.flatten(2, 3).transpose(1, 2)  # 水平扫描，直接 flatten
        elif scan_mode == 'V':
            x = x.transpose(2, 3).flatten(2, 3).transpose(1, 2)  # 竖直扫描，先 transpose 再 flatten
        elif scan_mode == 'D':
            x = diagonal_scan(x)
        ...
        if scan_mode =='H':
            ...
            x = x.transpose(1, 2).reshape(B, C, W // self.patch_size, H // self.patch_size).transpose(2, 3)
        elif scan_mode =='D':
            x = recover_from_diagonal_scan(x, B, C, H // self.patch_size, W // self.patch_size)
