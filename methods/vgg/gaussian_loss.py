...
def  meeloss(input, target, size_average=None, reduce=None, reduction='mean'):
    ...
    if target.requires_grad:
        # ret = 1 / torch.sqrt(2 * math.pi) * torch.exp(-torch.abs(input - target) ** 2 / 2)
        ret = torch.exp(-torch.abs(input - target) ** 2 / 2)
        # ret = torch.exp(-torch.abs(input - target) ** 2/2) / float(2 * ((0.1 * math.sqrt(2)) ** 2))
        # ret = 1 / torch.sqrt(2 * math.pi) * torch.exp(-torch.abs((input - target)*(input - target)) /2)
        if reduction != 'none':
            ret = torch.mean(ret) if reduction == 'mean' else torch.sum(ret)
    else:
        expanded_input, expanded_target = torch.broadcast_tensors(input, target)
        ret = torch._C._nn.mse_loss(expanded_input, expanded_target, _Reduction.get_enum(reduction))
        # ret =torch.exp(-torch.abs(input - target) ** 2 / 0.05 * math.sqrt(2))
        # ret = torch.mean(ret) if reduction == 'mean' else torch.sum(ret)
    return ret